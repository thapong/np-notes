...
export class OllamaProvider implements EmbeddingProvider {
  ...
  private async request(input: string | string[]) { const response = await fetch(`${this.config.ollamaBaseUrl}/api/embed`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: this.config.aiEmbeddingModel, input }), signal: AbortSignal.timeout(this.config.aiRequestTimeoutMs) }); if (!response.ok) throw new Error(`Ollama embedding failed: ${response.status}`); const data = await response.json() as { embeddings?: number[][] }; if (!data.embeddings?.length) throw new Error("Ollama returned no embeddings"); return data.embeddings; }
