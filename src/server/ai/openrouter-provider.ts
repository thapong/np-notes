...
export class OpenRouterProvider implements EmbeddingProvider {
  ...
  private async request(input: string | string[]) { const response = await fetch("https://openrouter.ai/api/v1/embeddings", { method: "POST", headers: { authorization: `Bearer ${this.config.openRouterApiKey}`, "content-type": "application/json" }, body: JSON.stringify({ model: this.config.aiEmbeddingModel, input }), signal: AbortSignal.timeout(this.config.aiRequestTimeoutMs) }); if (!response.ok) throw new Error(`OpenRouter embedding failed: ${response.status}`); const data = await response.json() as { data?: Array<{ embedding: number[] }> }; const vectors = data.data?.map((item) => item.embedding) ?? []; if (!vectors.length) throw new Error("OpenRouter returned no embeddings"); return vectors; }
